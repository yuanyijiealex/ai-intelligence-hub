...
def scan_inbox_for_alerts():
    ...
    if not inbox.exists():
        return []
    files = sorted([p for p in inbox.glob('*.md')] + [p for p in inbox.glob('*.txt')])
    return files[-10:]  # 最近 10 个作为扫描窗口
...
def main():
    ...
    ap.add_argument('--date', help='CST 日期 YYYY-MM-DD；默认今日', default=None)
    args = ap.parse_args()
    ...
    for p in candidates:
        ...
        if hits:
            ...
            summary = f"命中关键词 {len(hits)} 项（" + ", ".join([h[0] for h in hits]) + "）"
